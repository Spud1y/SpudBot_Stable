...
from staticvars import ADMIN_LIST
...
BAN_DIR=BASE_DIR_CONSTANT + "bans"
...
async def on_ready():
    ...
    if(exists(BAN_DIR)):
        print('bans repo already exists skipping creation')
        if(not exists(getBanFile())):
            print('repo exists but ban file does not... creating ban file')
            with io.open (getBanFile(), "w", encoding="utf-8") as f:
                f.close()
    else:
        print('ban repo does not exist, creating')
        os.makedirs(BAN_DIR)
        with io.open (getBanFile(), "w", encoding="utf-8") as f:
            f.close()

    updateBanList()
    print("ban list at startup is {}".format(BAN_LIST))
...
async def common(ctx):
    ...
    if command == 'ban' or command == 'unban':
        user = client.get_user(240732316270460928)
        await user.send("Someone is trying to ban with id {}".format(ctx.message.author.id))
        if(ctx.message.author.id == 240732316270460928):
            print("Spudly is personally banning")
        else:
            print("attempting to {} with id {}".format(command, ctx.message.author.id))
        return
    global BAN_LIST
    print("before invoke BAN_LIST = {}".format(BAN_LIST))
    if(str(ctx.message.author.id) in BAN_LIST):#'240732316270460928'): #second one is my id
        raise Exception("Yeah fuck that no more commands from this user")
    else:
        print("id of song player = {}".format(ctx.message.author.id))

@client.command(pass_context = True)
async def ban(ctx, *arg):
    if ctx.message.author.id in ADMIN_LIST:
        if(len(arg) > 1):
            await ctx.send("need only 1 argument of id to ban")
            return
        with io.open (getBanFile(), "a", encoding="utf-8") as f:
            f.writelines(arg[0] + "\n")
            f.close()
            await ctx.send("user `{}` got the ban :hammer:".format(arg[0]))
        updateBanList()
    else:
        await ctx.send("You don't have permissions to ban. Nice try :kissing_heart:")
...
async def unban(ctx, *arg):
    if ctx.message.author.id in ADMIN_LIST:
        if(len(arg) > 1):
            ...
            return
        banList = readBanList()
        with open(getBanFile(), "w", encoding="utf-8") as f:
            for ban in banList:
                if ban == arg[0]:
                    await ctx.send("unbanning {}".format(arg[0]))
                else:
                    f.write(ban)
                    f.write("\n")
        updateBanList()
...
def getBanFile():
    return BAN_DIR + OS_SEPARATOR + "banList" + ".txt"
...
def readBanList():
    banList= []
    with io.open (getBanFile(), "r", encoding="utf-8") as f:
        lines = f.readlines()
        for line in lines:
...
def updateBanList():
    global BAN_LIST
    BAN_LIST = readBanList()
    print("updating cached ban list... new list: \n{}".format(BAN_LIST))


""" ## once a song starts check back in every 10 minutes to see if bot needs to disconnect
